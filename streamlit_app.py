...
def solve_pipeline(FLOW, KV, rho, SFC_J, SFC_R, SFC_S, RateDRA, Price_HSD):
    ...
    # Prepare namespace
    local = dict(os=os, pyo=pyo, SolverManagerFactory=SolverManagerFactory,
...
if st.sidebar.button("Run Optimization"):
    with st.spinner("Optimizing via NEOS..."):
        model, local = solve_pipeline(FLOW, KV, rho, SFC_J, SFC_R, SFC_S, RateDRA, Price_HSD)
    st.success("Optimization Complete!")

    # Define stations and variable indices
stations = [
    ...
    for s in stations:
        ...
        if idx == "1":
            val = local.get('RH1')
            row["Residual Head (m)"] = pyo.value(val) if hasattr(val, 'is_expression') or hasattr(val, 'is_variable') else val
        else:
            try:
        ...
        sdh_key = f"SDHA_{idx}"
        val = local.get(sdh_key)
        row["Station Discharge Head (m)"] = pyo.value(val) if val is not None else None
        # Pump Efficiency
        if s["effp_idx"]:
            effp = local.get(f"EFFP{s['effp_idx']}")
            row["Pump Efficiency (%)"] = pyo.value(effp)
        else:
            row["Pump Efficiency (%)"] = None
        # Power Cost
        if s["power_idx"]:
            pc = local.get(f"OF_POWER_{s['power_idx']}")
            row["Power Cost (₹)"] = pyo.value(pc)
        else:
            row["Power Cost (₹)"] = None
        # DRA Cost
        if s["dra_idx"]:
            dc = local.get(f"OF_DRA_{s['dra_idx']}")
            row["DRA Cost (₹)"] = pyo.value(dc)
        else:
            row["DRA Cost (₹)"] = None
        rows.append(row)
    ...
    st.metric("Total Operating Cost (₹)", f"{total:,.2f}")
    ...
    st.markdown(
        "Enter inputs in the sidebar, click **Run Optimization**, and view pumps, drag reduction, speed, heads, efficiency, costs per station, plus total operating cost."
    )
